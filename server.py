...
def run_task(task_id, temp_path, db_path, client_name):
    """背景執行問卷比對，結果存入 tasks dict"""
    try:
        with tasks_lock:
            tasks[task_id]['status'] = 'processing'
        safe_log(f"[TASK] {task_id} 開始處理")
        ...
        safe_log(f"[TASK] {task_id} 完成，題數={report.get('total_questions', 0)}")

    except Exception as e:
        safe_log(f"[TASK] {task_id} 失敗: {repr(e)}")
...
@app.route("/api/security-c/submit", methods=["POST"])
def security_c_submit():
    """
    非同步提交：立即返回 task_id，後台開始比對
    前端再用 /api/security-c/status/<task_id> 輪詢進度
    """
    user = get_current_user()
    if not user:
        return jsonify({"success": False, "error": "SSO 未登入，請重新整理頁面"}), 401
    ...
    if not os.path.exists(DB_PATH):
        return jsonify({"success": False, "error": f"資料庫檔案不存在: {DB_PATH}"}), 500
    ...
    try:
        ...
        return jsonify({"success": False, "error": f"解碼 base64 失敗: {str(e)}"}), 400
    ...
    safe_log(f"[SUBMIT] 用戶 {user['username']} ({user['job_num']}) 提交任務 task_id={task_id}")
    return jsonify({"success": True, "task_id": task_id, "message": "任務已提交，請輪詢狀態"})


@app.route("/api/security-c/status/<task_id>", methods=["GET"])
def security_c_status(task_id):
    """查詢任務處理狀態"""
    with tasks_lock:
        task = tasks.get(task_id)

    if not task:
        return jsonify({"success": False, "error": "找不到此任務，可能已過期或 server 已重啟"}), 404
    ...
    if status == 'done':
        ...
        return jsonify({"success": True, "status": status})  # queued / processing
