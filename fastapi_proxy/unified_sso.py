...
class UnifiedSSO:
    ...
    def login(self, request: Request):
        ...
        # 備援：也設置 cookie
        resp.set_cookie("sso_next_path", quote(next_path, safe=""), max_age=300, path="/", samesite="none", secure=True)
    ...
    async def callback(self, request: Request):
        ...
        if request.method == "GET":
            ...
            cookie_next_raw = request.cookies.get("sso_next_path", "/")
            state = (form.get("state") or request.query_params.get("state") or "").strip()
            
            # 解碼 cookie（處理 URL 編碼）
            cookie_next = unquote(cookie_next_raw) if cookie_next_raw else "/"
            
            # 優先使用 cookie，fallback 到 state
            if cookie_next and cookie_next != "/":
                next_path = cookie_next
            elif state:
                next_path = unquote(state)
            else:
                next_path = "/"
            ...
            logger.info(f"[SSO] Callback - state={state or 'N/A'}, cookie_raw={cookie_next_raw}, cookie_decoded={cookie_next}, final_next={next_path}")
