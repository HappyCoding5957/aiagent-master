#!/home/lladm/frank/n8n/客戶問卷RPA/venv/bin/python3
# -*- coding: utf-8 -*-
"""
上傳附件三到 RAG 知識庫
...
PROGRESS_FILE = "/tmp/attachment3_upload_progress.json"
...
def main(attachment_path: str):
    ...
    for row_idx in range(2, ws.max_row + 1):
        a_col = ws[f"A{row_idx}"].value or ""  # 類別
        b_col = ws[f"B{row_idx}"].value or ""  # 行為準則
        c_col = ws[f"C{row_idx}"].value or ""  # 關鍵字
        d_col = ws[f"D{row_idx}"].value or ""  # 條文內容
        e_col = ws[f"E{row_idx}"].value or ""  # 權責部門
        f_col = ws[f"F{row_idx}"].value or ""  # 目前現況/可能影響
        g_col = ws[f"G{row_idx}"].value or ""  # 問卷出處
        ...
        if llm_cfg:
            try:
                ...
                # 檢查是否成功萃取（confidence > 0.0 表示有 LLM 萃取）
        ...
        # [print-A3ROW0][上面] 在組 chunk_text 前（抽樣用）
        if row_idx <= 4:
            print(f"  [print-A3ROW0][上面] row={row_id} behavior={str(b_col)[:20]} keywords={len(getattr(knowledge_item, 'keywords', []))}")

        # ✅ 組成結構化文字（包含 LLM 萃取的 intent 和 topic_tags）
        try:
        ...
        chunk_text = f"""[schema_json] {schema_json}
[row_id] {getattr(knowledge_item, 'row_id', row_id)}
[類別] {getattr(knowledge_item, 'category', str(a_col))}
[行為準則] {getattr(knowledge_item, 'behavior', str(b_col))}
[語意摘要] {getattr(knowledge_item, 'intent', '')}
[主題標籤] {', '.join(getattr(knowledge_item, 'topic_tags', []) or [])}
[關鍵字] {', '.join(getattr(knowledge_item, 'keywords', []) or [])}
[條文內容] {getattr(knowledge_item, 'article', str(d_col))}
[權責部門] {getattr(knowledge_item, 'dept', str(e_col))}
[目前現況] {getattr(knowledge_item, 'impact', str(f_col))}
[問卷出處] {getattr(knowledge_item, 'source', str(g_col))}
[信心度] {float(getattr(knowledge_item, 'confidence', 0.0)):.2f}"""
