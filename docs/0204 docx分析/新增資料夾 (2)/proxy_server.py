...
# 後端服務配置
UPLOAD_BACKEND = "http://127.0.0.1:4203"         # 上傳系統前端（Docker）
UPLOAD_API_BACKEND = "http://127.0.0.1:8001"    # 上傳系統後端 API（FastAPI Docker）
DOWNLOAD_BACKEND = "http://127.0.0.1:4204"      # 下載系統（Flask SSO）
    ...
    try:
        ...
        if location:
            ...
            if UPLOAD_BACKEND in location:
                location = location.replace(UPLOAD_BACKEND, 'http://ssw01.ennostar.com:4202/esg/upload')
            elif DOWNLOAD_BACKEND in location:
                location = location.replace(DOWNLOAD_BACKEND, 'http://ssw01.ennostar.com:4202/esg/download')
...
@app.route('/esg/upload/', defaults={'path': ''})
@app.route('/esg/upload/<path:path>')
