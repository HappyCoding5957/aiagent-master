...
def select_per_facet_candidates(candidates: list, facets: list) -> list:
    ...
    """
    facet_best = {}  # facet_term -> (candidate, score)
    ...
    for c in candidates:
        ...
        for term in facets:
            ...
            if cnt > 0:
                ...
                score = best_ng
            if score > 0:
                prev_score = facet_best.get(term, (None, -1))[1]
                if score > prev_score:
                    facet_best[term] = (c, score)

    # 去重：同一 row_id 只保留一次（保留分數最高的 facet 對應版本）
    seen_ids = set()
    result = []
    for _term, (c, _score) in sorted(facet_best.items(),
                                      key=lambda kv: -kv[1][1]):  # 分數高的先放
    ...
    try:
        ...
        if resp.status_code == 401:
            ...
            if "SSO" in error_detail or "登入" in error_detail:
                raise Exception(f"RAG 服務需要 SSO 認證，但服務間認證失敗。錯誤詳情：{error_detail}")
            else:
        ...
        print(f"  [RAG] ⚠️ RAG 搜尋失敗，將改用本地 keyword 補撈模式. 原因: {e}")
        data = []
...
def process_workbook(survey_path, db_path, client_name="未指定客戶"):
    ...
    for row_idx in range(start_row, ws.max_row + 1):
        ...
        if candidates:
            ...
            multifacet_tag = is_multifacet_question_expected(question, q_schema=q_schema)
            ...
            if selected is None:
                ...
                if multifacet_tag and len(facets_expected) >= 2:
                    ...
                    matches = selected["matches"]  # ⬅ 單面向：原邏輯不動
                    behavior = selected.get("behavior") or "無"
                    confidence = float(selected.get("best_score", 0.0)) * 100.0
                    article = selected.get("article") or ""
